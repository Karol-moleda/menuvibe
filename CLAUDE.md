...
- Aplikacja prywatna (jeden użytkownik), UI po polsku, Ionic standalone (`@ionic/angular` w v9 eksportuje komponenty standalone z głównego pakietu).
- Dane w Supabase; każda tabela ma `user_id` z domyślnym `auth.uid()` i RLS „tylko właściciel”. Automatyczne udostępnianie tabel jest wyłączone – nowa tabela w migracji wymaga `grant ... to authenticated`.
